...
export const updateProduct = async (req, reply) => {
  try {
    ...
    reply.status(200).send({ message: "Product successfully updated", product });
  } catch (error) {
    logger.error(`Failed to create product: ${"Error :",error}`);
...
export const filterProducts = async (req, reply) => {
  try {
    ...
    if (products.length === 0) {
      reply.status(404).send({ message: "No products found matching the specified criteria." });
    } else {
      reply.status(200).send(products);
    }
  ...
  const suggestions = await prisma.product.findMany({
    where: {
      OR: [
        ...
        { category: { name: { startsWith: query.substring(0, 3), mode: "insensitive" } } }
      ]
    },
    take: 5,
    include: {
