...
export const updateCategory = async (req, reply) => {
  try {
    ...
    reply.status(200).send({message: "Category updated sucessfully.", category});
  } catch (error) {
...
export const deleteCategory = async (req, reply) => {
  try {
    ...
    if (productsCount > 0 && !req.query.confirmed) {
      logger.info(`Category ${categoryId} has ${productsCount} products. Confirmation required`);
    ...
    if(productDelete) {
      logger.info(`Deleted ${productDelete.count} products related to category ID: ${categoryId}`);
    }
