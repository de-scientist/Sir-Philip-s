...
export const getOrdersByStatus = async (req, reply) => {
    try {
      ...
      if (orders.length === 0) {
        reply.status(404).send({ message: `No orders found with status: ${status}` });
      } else {
      ...
      reply.status(500).send({ message: "Failed to display the status records."});
    }
  };
...
export const deleteOrder = async (req, reply) => {
  try {
    ...
    reply.status(500).send({ message: "Failed to delete order.", error: error });
  }
};
...
export const deleteOrders = async (req, reply) => {
    ...
    if(confRecord.length === 0){
      ...
      if (deleteResult.count === 0) {
        ...
        reply.status(200).send({ message: `Deleted ${deleteResult.count} orders successfully` });
      }
    } catch (error) {
      ...
      if (error instanceof z.ZodError) {
        ...
        reply.status(500).send({ message: "Failed to delete orders", error: error });
      }
    }
  };